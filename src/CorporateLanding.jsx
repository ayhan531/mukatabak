...
const TICKERS = [
  ['XU100', 'BIST 100'], ['XU030', 'BIST 30'], ['USDTRY', 'Dolar'], ['EURTRY', 'Euro'], ['THYAO', 'THYAO'], ['ASELS', 'ASELS'],
];
...
function LiveTicker() {
  ...
  const rows = TICKERS.map(([code, label]) => [label, by.get(code)]).filter(([, q]) => q);
  if (!rows.length) return null;
  return (
    <div className="corporate-ticker">
      {rows.map(([label, q]) => {
        const up = Number(q.change_pct || 0) >= 0;
        return (
          <article key={label}>
            <span>{label}</span>
            <strong>{Number(q.price || 0).toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong>
            <em className={up ? 'up' : 'down'}>{(up ? '▲ +' : '▼ ') + '%' + Number(q.change_pct || 0).toFixed(2).replace('.', ',')}</em>
          </article>
        );
      })}
    </div>
...
export default function CorporateLanding({ openAuth }) {
  ...
  return <div className="corporate">
    <header className="corporate-nav"><a className="brand" href="/" onClick={e => { e.preventDefault(); go('Ana Sayfa'); }}>Mukatabak<i>+</i></a><nav className={menu ? 'open' : ''} aria-label="Kurumsal menü">{Object.keys(routes).filter(k => k !== 'Sözleşmeler').map(k => <a key={k} aria-current={page === k ? 'page' : undefined} href={routes[k]} onClick={e => { e.preventDefault(); go(k); }}>{k}</a>)}</nav><button className="corporate-login" onClick={openAuth}>E-Şube Giriş <ArrowUpRight size={16} /></button><button className="corporate-menu" aria-label="Menü" aria-expanded={menu} onClick={() => setMenu(!menu)}>{menu ? <X /> : <Menu />}</button></header>
    <main>{page === 'Ana Sayfa' ? <>
      <section className="corporate-hero"><div><span className="corporate-eyebrow">CANLI BIST · PORTFÖY · E-ŞUBE</span><h1>Yatırımın sade hâli</h1><h2>Mukatabak+ ile piyasa, portföy ve emirler tek ekranda.</h2><p>Canlı BIST fiyatlarını izleyin, hisse alıp satın, portföyünüzü ve T+2 bakiyenizi tek yerden yönetin. Telefonunuzda da tarayıcıda da aynı deneyim.</p><div className="corporate-actions"><button onClick={openAuth}>E-Şube'ye Giriş <ArrowRight size={18} /></button><button className="outline" onClick={() => go('Hizmetlerimiz')}>Hizmetlerimiz</button></div><div className="corporate-trust"><span><ShieldCheck size={17} /> İki adımlı güvenlik</span><span><Smartphone size={17} /> Telefona kurulabilir e-şube</span><span><Headphones size={17} /> Yatırımcı desteği</span></div></div></section>
      <LiveTicker />
      ...
      {page === 'Hakkımızda' && <><p className="lead">Yatırım dünyasına açılan dijital şubeniz.</p><p>Mukatabak Yatırım, piyasaları takip etmek ve hesap işlemlerini yönetmek için yatırımcıları e-şubede buluşturur. Portföy, emirler, hesap hareketleri ve haberler anlaşılır bir deneyimde bir araya gelir.</p><h2>Yatırımcı odaklı yaklaşım</h2><p>Varlık dağılımınızı ve işlem geçmişinizi görebilmek, yatırım sürecinizi takip etmenin temelidir. Mobil, tablet ve bilgisayarda aynı hesabınıza erişebilirsiniz.</p><h2>Şeffaf bilgi</h2><p>Yatırım ürünleri risk içerir. İşlem öncesinde ürün koşullarını, masrafları ve risk bildirimlerini incelemenizi öneririz.</p></>}
