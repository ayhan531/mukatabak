...
import { TradePanel, ReviewOrder, OrderResult, QuickTrade } from "./Trade.jsx";
import {
...
const NAV = [
  ...
  { title: "Hisseler", icon: "bars" },
  { title: "Al/Sat", icon: "swap" },
...
export default function App({ me, onLogout, onAdmin, onExit, refreshMe }) {
  /* ---- tercihler ---- */
  const [dark, setDark] = usePref("dark", false);   // APK varsayılanı: açık tema
  const [accent, setAccent] = usePref("accent", 1);   // varsayılan renk modu: Mavi
  const [skin, setSkin] = usePref("skin", 0);        // APK: Ayarlar > Tema
  ...
  useEffect(() => {
    ...
    root.dataset.skin = SKIN_KEYS[Math.min(SKIN_KEYS.length - 1, Math.max(0, skin))];
    const meta = document.querySelector('meta[name="theme-color"]');
    if (meta) meta.setAttribute("content", dark ? "#19202f" : "#e4e9ff");
  }, [dark, accent, textSize, skin]);
  ...
  /* ---- üst şerit ---- */
  const firstName = String(me?.full_name || "İsim").split(" ")[0];
  const brandBar = (
    <div className="brandbar">
      <div className="mk-word">Mukatabak<i>+</i></div>
      <div className="brandbar-actions">
        {me?.role === "admin" && (
          <button className="icon-btn round" onClick={onAdmin} title="Admin"><Icon name="shield" size={20} /></button>
        )}
      </div>
      <button className="mk-hello-wrap" onClick={() => setOverlay({ kind: "profile" })} aria-label={T("Profil")}
        style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <span className="mk-hello">
          <small>{T("Merhaba")}</small>
          <strong>{firstName}</strong>
        </span>
        <span className="mk-ava">{me?.avatar_url ? <img src={me.avatar_url} alt="" /> : monogram}</span>
      </button>
    </div>
  );
      ...
      case 3:
        return (
          <Portfolio
            ...
            onCreateOrder={() => openTrade(lastStock || listFor(BIST, market.instruments)[0], { searchable: true, sheet: true })}
          />
        );
      case 4:
        return (
  ...
  const navigate = (index) => {
    if (index === 2) {
      openTrade(lastStock || listFor(BIST, market.instruments)[0], { searchable: true, sheet: true });
      return;
    }
    go(index);
  };
...
function PushPrompt({ onClose, onNotice }) {
  ...
  const izinVer = async () => {
    ...
    else if (durum === "engellendi") onNotice("İzin verilmedi", "Ayarlar → Bildirimler bölümünden Mukatabak için bildirimlere izin verebilirsiniz.");
...
function InstallSheet({ onClose, onNotice }) {
  ...
  const elmaAdimlari = [
    ...
    ["check", "Sağ üstten “Ekle”ye dokunun", "Mukatabak simgesi ana ekrana gelir"],
  ];
