...
export const toInstrument = (quote) => {
  ...
  return {
    ...
    code: symbol,
    name: isFx ? CURRENCY_NAMES[symbol] || quote.name || symbol : shortName(symbol, quote.name),
    fullName: String(quote.name || symbol).trim(),
    price,
    change: changePct,
...
/**
 * APK'daki kısa şirket adı: resmî unvan yerine günlük kullanılan ad.
 * "TÜPRAŞ-TÜRKİYE PETROL RAFİNERİLERİ A.Ş." -> "Tüpraş"
 */
const SHORT_NAMES = {
  THYAO: "Türk Hava Yolları", TUPRS: "Tüpraş", ASELS: "Aselsan", AKBNK: "Akbank",
  GARAN: "Garanti BBVA", KCHOL: "Koç Holding", SISE: "Şişecam", SASA: "Sasa Polyester",
  BIMAS: "BİM Mağazalar", EREGL: "Ereğli Demir Çelik", ISCTR: "İş Bankası", YKBNK: "Yapı Kredi",
  SAHOL: "Sabancı Holding", FROTO: "Ford Otosan", TOASO: "Tofaş", PGSUS: "Pegasus",
...
const LEGAL = /\s*(A\.?\s?Ş\.?|ANON[İI]M\s+[ŞS][İI]RKET[İI]|SANAY[İI]|T[İI]CARET|HOLD[İI]NG|YATIRIM\s+ORTAKLI[ĞG]I|A\.O\.).*$/i;

export const shortName = (code, name = "") => {
  if (SHORT_NAMES[code]) return SHORT_NAMES[code];
  const raw = String(name || code || "").trim();
  if (!raw) return code || "";
