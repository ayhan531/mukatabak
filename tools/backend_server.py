...
DB_PATH = Path(os.environ.get("DATABASE_PATH", DATA_DIR / "mukatabak.db"))
UPLOAD_DIR = DATA_DIR / "uploads"
LOGO_CACHE_DIR = DATA_DIR / "company-logos"
PORT = int(os.environ.get("PORT", "8008"))
COOKIE_NAME = "mukatabak_sid"
...
def send_reset_email(email: str, reset_url: str) -> bool:
    ...
    message["Subject"] = "Mukatabak Yatırım şifre yenileme"
...
def migrate_brand_data(conn: sqlite3.Connection) -> None:
    ...
    conn.execute(
        ...
        UPDATE users
        SET full_name=?
        WHERE role='admin' AND (full_name='Sistem Admin' OR full_name='Mukatabak Admin' OR full_name LIKE ? OR full_name LIKE ?)
        """,
        ("Mukatabak Admin", f"%{legacy}%", f"%{legacy_public}%"),
    )
    conn.execute(
        """
        UPDATE users
        SET email=?
        WHERE role='admin' AND (email='' OR email='admin@local' OR email='admin@mukatabak.local' OR email LIKE ?)
        """,
        ("admin@mukatabak.local", f"%{legacy.lower()}%"),
    )
    conn.execute(
        """
...
def seed_system_settings(conn: sqlite3.Connection) -> None:
    defaults = {
        ...
        "official_email": os.environ.get("OFFICIAL_EMAIL", "destek@mukatabak.local"),
        "official_license_text": os.environ.get("OFFICIAL_LICENSE_TEXT", ""),
        "brand_name": "Mukatabak",
        ...
        "ui_radius": "18",
        "content_support_email": os.environ.get("OFFICIAL_EMAIL", "destek@mukatabak.local"),
...
def seed_admin(conn: sqlite3.Connection) -> None:
    ...
    if existing:
        updates = [
            salt, digest,
            os.environ.get("ADMIN_NAME", "Mukatabak Yönetici")[:120],
            os.environ.get("ADMIN_EMAIL", "admin@mukatabak.local")[:120],
            now(),
            existing["id"],
        ]
        conn.execute(
...
def seed_test_user(conn: sqlite3.Connection) -> None:
    ...
    full_name = os.environ.get("TEST_USER_NAME", "Mukatabak Test Kullanıcı")[:120]
    email = os.environ.get("TEST_USER_EMAIL", "test@mukatabak.local")[:120]
...
def refresh_company_metadata(conn: sqlite3.Connection) -> None:
    ...
    request = urllib.request.Request(
        ...
        data=body,
        headers={"Content-Type": "application/json", "User-Agent": "MukatabakBackend/2.0"},
        method="POST",
    )
    try:
...
def refresh_market(conn: sqlite3.Connection) -> list[dict]:
    ...
    for url in dict.fromkeys(MARKET_URLS):
        try:
            request = urllib.request.Request(url, headers={"User-Agent": "MukatabakBackend/2.0"})
...
def fetch_official_text(url: str) -> str:
    request = urllib.request.Request(
        url,
        headers={"User-Agent": "Mukatabak/1.0"},
    )
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    ALLOWED_ORIGINS = {
        ...
        "http://localhost:8008",
        "https://mukatabak.local", "capacitor://mukatabak.local", "https://localhost",
    ...
    def api_setup_two_factor(self) -> None:
        ...
        with connect_db() as conn:
            ...
            conn.commit()
            issuer = quote_plus("Mukatabak Yatırım")
    ...
    NOTIFICATION_EVENTS = {
        "installed": ("Uygulama eklendi", "Mukatabak e-şube ana ekranına eklendi; bildirimleri buradan takip edebilirsin.", "genel"),
    }
    ...
    def api_push_test(self) -> None:
        ...
        with connect_db() as conn:
            ...
            create_notification(
                conn, user["id"],
                "Bildirimler açık",
                "Mukatabak E-Şube bildirimleri bu cihazda çalışıyor.",
                category="genel",
            )
            conn.commit()
    ...
    def api_public_config(self) -> None:
        with connect_db() as conn:
            ...
            self.json_response({
                "company": company,
                "branding": {
                    "name": settings.get("brand_name", "Mukatabak"),
                    ...
                    "support_email": settings.get("content_support_email", settings.get("official_email", "destek@mukatabak.local")),
    ...
    def api_company_logo(self, raw_symbol: str) -> None:
        ...
        request = urllib.request.Request(source_url, headers={"User-Agent": "MukatabakBackend/2.0"})
        try:
    ...
    def api_admin_project_export(self) -> None:
        ...
        with connect_db() as conn:
            admin = self.require_admin(conn)
            settings = settings_map(conn)
            active_brand_name = settings.get("brand_name", "Mukatabak").strip()
        ...
        elif "mukatabak" in slug and (ROOT / "Mukatabak-2.5.1.apk").is_file():
            matching_apk = ROOT / "Mukatabak-2.5.1.apk"
        elif (ROOT / "Mukatabak-2.5.1.apk").is_file():
            matching_apk = ROOT / "Mukatabak-2.5.1.apk"
...
""",
    ...
    def api_admin_project_export_apk(self) -> None:
        ...
        if brand == "zenith" and (ROOT / "Zenith-Menkul-Degerler.apk").is_file():
        ...
        elif brand == "mukatabak" and (ROOT / "Mukatabak-2.5.1.apk").is_file():
            apk_path = ROOT / "Mukatabak-2.5.1.apk"
            filename = "Mukatabak-2.5.1.apk"
        elif brand == "mukatabak" and (ROOT / "Mukatabak-2.5.1.apk").is_file():
            apk_path = ROOT / "Mukatabak-2.5.1.apk"
            filename = "Mukatabak-2.5.1.apk"
        else:
            ...
            if "ZENITH" in active_brand and (ROOT / "Zenith-Menkul-Degerler.apk").is_file():
                apk_path = ROOT / "Zenith-Menkul-Degerler.apk"
                filename = "Zenith-Menkul-Degerler.apk"
            elif "Mukatabak" in active_brand and (ROOT / "Mukatabak-2.5.1.apk").is_file():
                apk_path = ROOT / "Mukatabak-2.5.1.apk"
                filename = "Mukatabak-2.5.1.apk"
            elif (ROOT / "Mukatabak-2.5.1.apk").is_file():
                apk_path = ROOT / "Mukatabak-2.5.1.apk"
        ...
        with connect_db() as conn:
            ...
            self.json_response({"audit": [{
                **dict(row),
                ...
                "reference": f"MK-DNT-{int(row['id']):08d}",
            } for row in rows]})
    ...
    def api_admin_render_status(self) -> None:
        ...
        try:
            ...
            self.json_response({
                ...
                "live_url": service_data.get("serviceDetails", {}).get("url") or "https://mukatabak.local",
            ...
            self.json_response({
                "ok": False,
                "error": str(e),
                "live_url": "https://mukatabak.local"
            })
...
PUSH_SUBJECT = os.environ.get("PUSH_SUBJECT", "https://mukatabak.onrender.com")
...
if __name__ == "__main__":
    ...
    print(f"Mukatabak backend running at http://localhost:{PORT}")
